...
const Pagos = () => {
  ...
  const cargarEventos = async () => {
    try {
      const filtros = {};
      if (hasRole(usuario?.rol, [ROLES.COORDINATOR]) && usuario?.id) {
        filtros.coordinador_id = usuario.id;
      }
      const data = await eventosService.getAll(filtros);
  ...
  const cargarPagos = async () => {
    try {
      setLoading(true);
      const data = await pagosService.getByEvento(filtroEvento);
      setPagos(data.pagos || []);
      setTotalPagado(data.total_pagado || 0);
      try {
        const totales = await pagosService.getTotalEvento(filtroEvento);
  ...
  const cargarInfoEvento = async () => {
    try {
      const evento = eventos.find((e) => e.id_evento === parseInt(filtroEvento));
      if (evento) {
        setEventoSeleccionado(evento);
      }
    } catch (err) {
      console.error('Error al cargar información del evento:', err);
    }
  };
  ...
  const abrirModalRegistrar = async (eventoOverride = null, tipoPago = 'abono') => {
    const eventoBase = eventoOverride || eventoSeleccionado;
    const eventoId = eventoBase?.id_evento || eventoBase?.id || filtroEvento;
    if (!eventoBase || !eventoId) {
      warning('Por favor seleccione un evento primero');
      return;
    }
    if (tipoPago !== 'reembolso') {
      const saldoPendiente = eventoBase?.saldo ?? calcularSaldoPendiente();
      ...
      try {
        const totales = await pagosService.getTotalEvento(eventoId);
  ...
  const abrirAdicionalesEvento = () => {
    ...
    const redirect = `/eventos/${eventoSeleccionado.id_evento || eventoSeleccionado.id}?tab=productos`;
  ...
  const seleccionarEvento = (evento) => {
    ...
    setFiltroEvento(String(eventoId));
    setEventoSeleccionado(evento);
    const total = parseNumero(evento.total || 0);
    const saldo = parseNumero(evento.saldo || 0);
    setTotalPagado(Math.max(0, total - saldo));
  };

  const eventosFiltrados = eventos
    .filter((evento) => {
      if (!busquedaEvento) return true;
      const texto = normalizarTexto(busquedaEvento);
      return (
        normalizarTexto(evento.nombre_evento).includes(texto) ||
        normalizarTexto(evento.nombre_cliente).includes(texto) ||
        normalizarTexto(evento.tipo_evento).includes(texto)
      );
    })
    .filter((evento) => (filtroEstadoEvento === 'todos' ? true : evento.estado === filtroEstadoEvento))
    .sort((a, b) => new Date(b.fecha_evento).getTime() - new Date(a.fecha_evento).getTime());

  const metricasEventos = eventosFiltrados.reduce(
    (acc, evento) => {
      const total = parseNumero(evento.total || 0);
      const saldo = parseNumero(evento.saldo || 0);
      acc.totalEventos += 1;
      ...
      acc.totalPagado += Math.max(0, total - saldo);
      return acc;
    },
    { totalEventos: 0, totalFacturado: 0, totalPagado: 0, totalSaldo: 0 }
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>Pagos</h1>
          <p style={{ color: '#6b7280' }}>Gestión de pagos y abonos</p>
        </div>
      </div>
      ...
      >
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: '600' }}>Eventos</div>
          <div style={{ fontSize: '1.5rem', fontWeight: '700' }}>{metricasEventos.totalEventos}</div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: '600' }}>Total facturado</div>
          <div style={{ fontSize: '1.3rem', fontWeight: '700', color: '#6366f1' }}>
            {formatearMoneda(metricasEventos.totalFacturado)}
          </div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: '600' }}>Total pagado</div>
          <div style={{ fontSize: '1.3rem', fontWeight: '700', color: '#10b981' }}>
            {formatearMoneda(metricasEventos.totalPagado)}
          </div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ fontSize: '1.3rem', fontWeight: '700', color: '#ef4444' }}>
            {formatearMoneda(metricasEventos.totalSaldo)}
          </div>
        </div>
      </div>
      ...
      >
        <div style={{ position: 'relative', flex: 1, minWidth: isMobile ? '100%' : '260px' }}>
          ...
          <input
            type="text"
            placeholder="Buscar por evento, cliente o tipo..."
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
            <div>
              ...
              <p style={{ margin: 0, fontSize: '1rem', fontWeight: '600' }}>{eventoSeleccionado.nombre_evento}</p>
              <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: '#6b7280' }}>
                {eventoSeleccionado.nombre_cliente}
              </p>
            </div>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
              ...
              <p style={{ margin: 0, fontSize: '1.25rem', fontWeight: 'bold', color: '#6366f1' }}>
                {formatearMoneda(parseFloat(eventoSeleccionado.total || 0))}
              </p>
            </div>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {puedeRegistrar && (
                <button
                  type="button"
                  onClick={() => abrirModalRegistrar(eventoSeleccionado, 'abono')}
                  disabled={!eventoSeleccionado || !tieneSaldoPendiente}
                  style={{
                    padding: '0.5rem 0.85rem',
                    borderRadius: '0.375rem',
                    border: 'none',
                    backgroundColor: !eventoSeleccionado || !tieneSaldoPendiente ? '#9ca3af' : '#10b981',
                    color: 'white',
                    cursor: !eventoSeleccionado || !tieneSaldoPendiente ? 'not-allowed' : 'pointer',
                    fontSize: '0.8rem',
                    fontWeight: '600',
                  }}
                >
              ...
              {puedeReembolsar && (
                <button
                  ...
                  disabled={!eventoSeleccionado || saldoPorReembolsar <= 0}
                  style={{
                    padding: '0.5rem 0.85rem',
                    borderRadius: '0.375rem',
                    border: 'none',
                    backgroundColor: !eventoSeleccionado || saldoPorReembolsar <= 0 ? '#fca5a5' : '#ef4444',
                    color: 'white',
                    cursor: !eventoSeleccionado || saldoPorReembolsar <= 0 ? 'not-allowed' : 'pointer',
                    fontSize: '0.8rem',
                    fontWeight: '600',
                  }}
                >
            ...
            <form onSubmit={handleRegistrarPago}>
              <div style={{ display: 'grid', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    type="text"
                    value={eventos.find((e) => e.id_evento === parseInt(filtroEvento))?.nombre_evento || ''}
                    disabled
                    style={{
                      width: '100%',
                      padding: '0.75rem',
